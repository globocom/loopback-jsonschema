...
describe('schemaBodyUrlRewriter', function() {
    describe('.makeAbsolute', function() {
        describe('when originalBody is an object', function() {
            var body = {};
            var originalBody = {};
            var ljsReq;
            ...
            describe('when links, properties and items do not exist', function() {
                ...
                it('should do nothing', function() {
                    body = schemaBodyUrlRewriter.makeAbsolute(ljsReq, {});
        ...
        describe('when originalBody is an array', function() {
            var body = [];
            var originalBody = [
                {
                    links: [
                        { schema: { $ref: '/relative' } },
                        { schema: { $ref: '/relative2' } }
                    ]
                },
                {
                    links: [
                        { schema: { $ref: '/relative3' } },
                        { schema: { $ref: '/relative4' } }
                    ]
                }
            ];
            ...
            it('should make absolute in each array element', function() {
                body = schemaBodyUrlRewriter.makeAbsolute(ljsReq, originalBody);

                expect(body[0].links[0].schema.$ref).to.eq('http://example.org/relative');
                expect(body[0].links[1].schema.$ref).to.eq('http://example.org/relative2');
                expect(body[1].links[0].schema.$ref).to.eq('http://example.org/relative3');
                expect(body[1].links[1].schema.$ref).to.eq('http://example.org/relative4');
            });
        });
    });
});
