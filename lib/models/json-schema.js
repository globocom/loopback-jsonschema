...
JsonSchema.defaultLinks = function(ljsReq, jsonSchema) {
    var href = ljsReq && (ljsReq.baseUrl() + '/' + jsonSchema.collectionName + '/{id}');
    var collectionUrl = ljsReq && (ljsReq.baseUrl() + '/' + jsonSchema.collectionName);
    var schemaUrl = ljsReq && (ljsReq.baseUrl() + '/' + jsonSchema.constructor.pluralModelName + '/' + jsonSchema.id)
    return [
        { rel: 'self', href: href },
        { rel: 'item', href: href },
...
JsonSchema.on('attached', function(app) {
    JsonSchema.afterRemote('**', function(ctx, result, next) {
        if (ctx.result) {
            ...
            var defaultLinks = JsonSchema.defaultLinks(ljsReq, result);
