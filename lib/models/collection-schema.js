...
CollectionSchema.prototype.links = function(itemSchemaUrl, collectionUrl) {
    var defaultLinks = [
        {
            rel: 'self',
            href: collectionUrl
    ...
    var links = new JsonSchemaLinks(this.ljsReq, defaultLinks);
    return links.all();
}
...
CollectionSchema.prototype.data = function(callback) {
    ...
    findBySchemaId(this.id, function(err, itemSchema) {
        ...
        if (itemSchema) {
            ...
            var links = self.links(itemSchemaUrl, collectionUrl);
            if (links) {
                schema.links = links;
            }
        }
