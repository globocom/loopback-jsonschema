...
CollectionSchema.prototype.links = function(collectionSchemaUrl) {
    return [
        { rel: 'self', href: collectionSchemaUrl }
    ];
}
...
CollectionSchema.prototype.data = function(callback) {
    ...
    findBySchemaId(this.id, function(err, itemSchema) {
        ...
        if (itemSchema) {
            ...
            var collectionSchemaUrl = LJSUrl.buildFromModel(self.ljsReq, self).url;
            ...
            var links = self.links(collectionSchemaUrl);
            if (links) {
                schema.links = links;
            }
        }
