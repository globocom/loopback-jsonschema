...
module.exports = function readOnlyDefaultValuesHandler(ctx) {
    ...
    return schemaWalk(properties, [], traversePayload, methodName);
};
...
function schemaWalk(properties, propertyPath, traversePayload, methodName) {
    var bodyItems;
    var nestedProperties;
    var path;
    var property;
    ...
    for (var key in properties) {
        ...
        if (property.default) {
            applyDefaultPropertyValue(traversePayload, path, property.default, methodName);
        }
    }
...
function applyDefaultPropertyValue(traversePayload, path, defaultValue, methodName) {
    var value = traversePayload.has(path);

    if (methodName !== 'updateAttributes' && !value) {
