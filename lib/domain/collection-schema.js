...
CollectionSchema.prototype.links = function(customLinks, itemSchemaUrl) {
    ...
    var defaultLinks = [
        ...
        {
            rel: 'previous',
            href: collectionUrl + '?filter[limit]={limit}&filter[offset]={previousOffset}&{+qs}'
        },
        {
            rel: 'next',
            href: collectionUrl + '?filter[limit]={limit}&filter[offset]={nextOffset}&{+qs}'
        },
        {
            rel: 'order',
            href: collectionUrl + '?filter[order]={orderAttribute} {orderDirection}&{+orderQs}'
        }
    ];
...
CollectionSchema.prototype.url = function() {
    return '/' + this.constructor.pluralModelName + '/' + this.itemSchema.id;
};
