...
function bindRelations(fromCollectionName, fromModel, relations) {
    ...
    for (var relationKey in relations) {
        ...
        if (!toModel) {
            logger.debug('[bindRelation] collectionName: "'+relation.collectionName+'" not found, storing as pending relation');
...
function makeAssociation(fromModel, toModel, relationKey, relation) {
    ...
    // reload modules
    // TODO: release this workaround
    var newFromModel = loopback.findModel(fromModel.modelName) || fromModel;
    var newToModel = loopback.findModel(toModel.modelName) || toModel;

    if (relation.type == 'belongsTo') {
        definition = newFromModel.belongsTo(newToModel, {as: relationKey, foreignKey: relation.foreignKey});
    } else if (relation.type == 'hasMany') {
        definition = newFromModel.hasMany(newToModel, {as: relationKey, foreignKey: relation.foreignKey});
    }

    this.emit('association', relation.type, definition);

    bindAssociationHooks.call(this, newFromModel, newToModel, relationKey, relation);
}
