...
function sanitizeKeys(indexes, operation) {
    for (var indexObject in indexes) {
        var index = indexes[indexObject];
        var sanitizedKeys = operation(getIndexKeys(indexes[indexObject]))

        // kudos on consistency on your data structures ...
        if('keys' in index) {
            index.keys = sanitizedKeys;
        } else {
            indexes[indexObject] = sanitizedKeys;
        }
    }
}
