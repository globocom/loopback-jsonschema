...
ItemSchema.prototype.associateModel = function(model, callback) {
    if (this.relations) {
        bindRelations(model, this.relations);
    }

    bindPendingRelations(this.collectionName, model);
};
...
function bindRelations(targetCollectionName, targetModel, relations) {
    ...
    for (var relationKey in relations) {
        ...
        relationModel = models.fromPluralModelName(ItemSchema.app, relation.collectionName);

        if (!relationModel) {
            logger.info('[bindRelation] collectionName: "'+relation.collectionName+'" not found');
            pushPendingRelation(targetCollectionName, relation.collectionName, relationKey, relation);
            return;
        }
...
function bindPendingRelations(relationCollectionName, relationModel) {
    var pendingRelations = ItemSchema.pendingRelations[relationCollectionName];
    var pendingRelation;
    ...
    for (var i=0; i<pendingRelations.lenght; i++) {
        pendingRelation = pendingRelations[i];
        console.info('>>>>>>>', pendingRelation);
    }
};

function pushPendingRelation(targetCollectionName, relationCollectionName, relationKey, relation) {
    if (!ItemSchema.pendingRelations[relationCollectionName]) {
        ItemSchema.pendingRelations[relationCollectionName] = [];
    }

    ItemSchema.pendingRelations[relationCollectionName].push({
