...
ItemSchema.prototype.beforeRegisterLoopbackModel = function(app, JsonSchemaModel, callback) {
    ...
    if (ItemSchema.app._registeredLoopbackHooks[JsonSchemaModel.modelName]) {
        ...
        this.defineRemoteHooks(JsonSchemaModel);
...
ItemSchema.prototype.defineRemoteHooks = function(model) {
    logger.debug('Defining remote hooks for ' + model.modelName);
    var hooksInitializer = ItemSchema.modelHooksInitializer;
    var schema = this;

    for (var i=0; i<hooksInitializer.length; i++) {
        hooksInitializer[i].call(null, schema, model);
    }
};


ItemSchema.modelHooksInitializer = [
    includeBodyFieldInjector
];

ItemSchema.registerModelHooksInitializer = function(hooks) {
    if (hooks instanceof Array) {
        ItemSchema.modelHooksInitializer = ItemSchema.modelHooksInitializer.concat(hooks);
    } else {
        ItemSchema.modelHooksInitializer.push(hooks);
    }
};
...
function includeBodyFieldInjector (schema, model) {
