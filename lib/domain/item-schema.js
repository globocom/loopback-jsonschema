...
ItemSchema.prototype.createIndexes = function() {
    if(this.getDataSource().connector.name.toLowerCase() === "mongodb"){
        logger.info('Start creating indexes dynamically.');
        this.getDataSource().connector.autoupdate([this.modelName], function ensureIndexCallback(err) {
            if (err) {
                logger.error('Error ensuring dynamic index. Error:', err);
                return;
            }
            logger.info('Ensured MongoDB dinamic index.');
        });
    }
};
