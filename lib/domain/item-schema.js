...
ItemSchema.prototype.beforeRegisterLoopbackModel = function(app, JsonSchemaModel, callback) {
    var name = JsonSchemaModel.sharedClass.name;
...
ItemSchema.preLoadModels = function(currentAttemptNumber) {
    ...
    ItemSchema.find({}, function(err, itemSchemas) {
        ...
        for(var i = 0; i < itemSchemas.length; i++) {
            ...
            registryModels.appendModelV1(schema.collectionName, model);

            var modelV2 = schema.constructModelV2();
            registryModels.appendModelV2(schema.collectionName, 'default', modelV2);

            ItemSchema.attachModel(model);
            promisses.push(registerModel(schema, model));

            ItemSchema.attachModel(modelV2);
            promisses.push(registerModel(schema, modelV2));

            var tenantsPoolMap = schema.modelTenantsPool(model);

            tenantsPoolMap.forEach((tenantId) => {
                var scopedModel = schema.constructModelV2(tenantId);
                registryModels.appendModelV2(schema.collectionName, tenantId, scopedModel);
                scopedModel.tenantId = tenantId;
                ItemSchema.attachModel(scopedModel);
                promisses.push(registerModel(schema, scopedModel));
            });
        }
