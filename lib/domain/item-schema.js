...
ItemSchema.prototype.constructModel = function() {
    removeOldModelInRegistry(this.modelName);
...
function removeOldModelInRegistry(modelName) {
    var oldModel = loopback.findModel(modelName);
    if (oldModel) {
        // unbind old model
        var models = ItemSchema.app.models();
        var position = models.indexOf(oldModel);
        models.pop(position);
    }
}
