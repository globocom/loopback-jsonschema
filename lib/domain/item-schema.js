...
ItemSchema.afterLoadModels = function(mapSchemas, mapModels, next) {
    next();
};
...
ItemSchema.preLoadModels = function() {
    ...
    ItemSchema.find({}, function(err, itemSchemas) {
        ...
        itemSchemas.forEach(function(itemSchema) {
            ...
            mapSchemas[itemSchema.modelName] = itemSchema;
            ItemSchema.attachModel(model);
        });

        ItemSchema.afterLoadModels(mapSchemas, mapModels, function() {
            var promisses = _.map(mapSchemas, function(schema, modelName) {
                return registerModel(schema, mapModels[modelName]);
            });
