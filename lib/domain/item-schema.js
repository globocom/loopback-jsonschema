...
ItemSchema.observe('before save', function beforeSave(ctx, next) {
    var schema = ctx.instance || ctx.data;

    if(!schema.weakValidation) {
        this.errors = extendedValidation(schema);

        if(this.errors.length) {
            next('Your API schema is invalid. Fix these violations: ' + this.errors.join('; '));
        }
...
function extendedValidation(schema) {
    var errors = validateProperties(schema);
